...
public:
    ...
    static std::vector<std::tuple<std::string, std::string, std::string>> getMessages(const std::string& receiver) {
        ...
        std::string query = "SELECT SENDERNAME, MESSAGE, TIMESTAMP FROM MESSAGES WHERE RECEIVERNAME = ?";
        sqlite3_stmt* stmt;
        rc = sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr);
        ...
        rc = sqlite3_bind_text(stmt, 1, receiver.c_str(), -1, SQLITE_STATIC);
        ...
        while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            messages.push_back(std::make_tuple(sender, message, timestamp));
        }
    ...
    static std::string vectorTupleToString(const std::vector<std::tuple<std::string, std::string, std::string>>& vecTuple) {
        std::stringstream result;

        for (const auto& tpl : vecTuple) {
            // Извлекаем элементы кортежа
            std::string sender = std::get<0>(tpl);
            std::string message = std::get<1>(tpl);
            std::string timestamp = std::get<2>(tpl);

            // Формируем строку и добавляем к результату
            result << sender << "|" << message << "|" << timestamp << "\n";
        }
