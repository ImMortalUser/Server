...
{
public:
    static std::string handle_request(std::string answer, std::string ip, SOCKET clientSocket, std::string& username, bool& logged, std::map<std::string, SOCKET>& clientList)
    {
        ...
        else if (request.find("<wtcw>") != std::string::npos)
        {
            ...
            {
                SOCKET friendSocket = clientList[friend_];
                for (;;)
                {
                    std::string message = TCPSocket::receiveFromClient(clientSocket);
                    if (message == "<exit>")
        ...
        {
            std::cout << "request_not_recognized\n";
            return "0";
        }
        return "-1";
    }
